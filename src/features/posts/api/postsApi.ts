...
export const postsApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    updatePost: builder.mutation<void, { postId: number; description: string }>({
      ...
      invalidatesTags: ['posts', 'ProfilePosts'],
      async onQueryStarted({ postId, description }, { dispatch, queryFulfilled }) {
    ...
    getProfilePosts: builder.query<Posts, { profileId: string; pageNumber: number }>({
      query: ({ profileId, pageNumber = 1 }) =>
        `/posts/users/${profileId}?pageNumber=${pageNumber}`,
      serializeQueryArgs: ({ endpointName }) => endpointName,
      merge: (currentCacheData, responseData) => {
        if (responseData.page === 1) {
          currentCacheData.items = responseData.items.slice();
        } else {
