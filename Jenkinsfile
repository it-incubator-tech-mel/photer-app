...
pipeline {
    ...
    environment {
        ENV_TYPE = "production"
        PORT = 3740
        NAMESPACE = "photer-ltd"
        REGISTRY_HOSTNAME = "photer"
        REGISTRY = "registry.hub.docker.com"
        PROJECT = "photer-inctagram"
        DEPLOYMENT_NAME = "photer-inctagram-deployment"
        ...
        stage('Push docker image') {
             steps {
                 echo "Push image started..."
                     script {
                          docker.withRegistry("https://${env.REGISTRY}", 'photer-ltd') {
